...
export function buildCity(THREE, scene){
  ...
  (function buskerStage(){
    ...
    /* 麥克風架站在舞台前緣、面對巡邏路人常走的那條人行道(不是舞台正中央,
       ...
       這裡往前推 .9 個單位剛好在他身前)。 */
    const micX = bx + Math.sin(rot)*.9, micZ = bz + Math.cos(rot)*.9;
  ...
  /* ===== 永安街對面(z 100 以南)的空隙(2026-09-15,kc:「你外圍都檢查一次」,
     ...
     那一側只有警察局(x -84.5~-71.5)、公車亭(x -52~-20)、工地(x 2~38)三個
     地標。第一版把地標之間也塞滿街屋,kc 打回:「工地、公車站、警局中間的
     空格是故意的,你要補的是他們兩側的非重點住宅區」——所以只補兩翼:警察局
     西邊到地圖邊界、工地東邊到地圖邊界,面朝永安街(z 100~111),格子對不上
     的零頭用 fillerHouse() 補窄屋貼齊。地標之間跟它們背後那條空帶維持裸地
     (kc 的決定),工地內部本來就是裸地。 */
  ...
  row({ axis:'x', at:YA_AT, face:-1, from:-94, shops:[ S.sh ]});                      // 警察局西翼:-99.8~-88.2
  ...
  /* 三個地標之間那兩塊空地圍起來(2026-09-15,kc:「警察局那一排幫我把空格的
     ...
     上立一道空地圍牆:水泥牆板 3.2 高、每 6.5 個單位一根略高的方柱,台灣路邊
     圍起來的畸零地就長這樣。
     第一版立在街面線 z=100,kc 打回:「圍牆位置放錯了,是放在正方體的後面的
     區塊,不然你會擋到警車之類的」——警車就停在警察局東邊那塊空地上(x -70~-55,
     z 100~105),牆立在街面線會從車子前面切過去。改到 z=113(警察局那個方塊的
     背面線),空地整塊留在牆前面繼續停車,牆只負責擋住背後那條空帶的視線。
     第二版分成 A、B 兩段,中間讓開公車亭;kc:「公車站後面的圍牆要連接起來」
     ——公車亭背牆只到 z≈107,z=113 這條線在它後面是通的,兩段中間斷開會看到
     缺口。改成一整道從警察局東牆(-71.5)拉到工地西緣(2),從公車亭後面直接
     穿過去。兩端各多伸 1 個單位塞進隔壁量體裡,轉角不留縫。 */
  function vacantLotWall(x0, x1){
    const Z = 113, H = 3.2, T = .5, PITCH = 6.5;
  ...
  (function taipingExitCaps(){
    ...
    /* 巷口朝永安街那面換成素面水泥(2026-09-22,kc:「兩棟大樓中突然變成破舊
       磚瓦?」)——兩塊補牆+巷子末段兩道牆的端面,從街上看是一整片 28 寬的
  ...
  /* 小狗搬到校門口(2026-09-22,kc:「狗改放到學校附近好了」)——原本跟貓一起在
     廟埕(28,-90)。校門(-36,70.4)西邊人行道,離門 7 以上避開 nearDoor 半徑 3.6,
     教官站東邊(-29,71)不會疊到。人行道頂面 y=.3,跟廟埕石板剛好同高。 */
  /* 朝向:dog.glb 鼻子在模型 +z 那端(2026-09-22 拆 glb 頂點量的:z 正端最高點 506、
     腳不落地=頭;負端是尾巴),ry=0 就是面向 +z=馬路。之前兩次(π/2、π)都是看截圖
     猜的,鏡頭方向猜反了,以後模型朝向直接量,不看截圖猜。 */
  placePet('dog.glb', -43, 72, '小狗', 1.8, .3, 0);   // 2026-08-14 kc 說「狗狗要大隻一點」,1.3→1.8
