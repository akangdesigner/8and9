...
**`planter-bushes.glb`(2026-09-04,kc:「樓梯跟花圃有沒有3d模型」→
「先給我花圃/樓梯」):** 醫院獨立建築(見 `scene-city3d.js`
`hospitalCompound()`)入口的花圃原本是灰模箱體,跟公車站長椅/垃圾桶
同一個理由換真模型,挑跟現有木箱造型接近的「Planter & Bushes」
(J-Toastie,跟本專案 litter-lunchbox.glb 同作者)。台階當時也一起下載
了 Quaternius 的「Stairs」,但同一天第十四輪 kc 說「樓梯刪掉」——
`stairs.glb` 已經從 repo 刪掉,這裡不留這筆紀錄誤導以後的人去找一個
不存在的檔案。
