...
單一全域物件 `ST`,原本五條固定指標,2026-08-13 kc 兩次推翻「五條上限」
補到七條:`money`(金錢)／`full`(飽足)／`calm`(歸屬感——顯示名稱已改,
程式 key 還沒改,見 DESIGN_NOTES「實作現況」)／`cool`(帥潮)／`mood`
(情緒,第六條)／`stamina`(體力,第七條)／`star`(通緝,上限 5)。改動
指標一律走 `chg(k, d)`,不要直接寫 `ST.xxx = ...`——`chg` 負責 clamp 範圍、
更新 HUD、跳飄字。七條指標各自「只由什麼改變」的邊界記在 DESIGN_NOTES
「五條指標各自管什麼」+「第六條:情緒」+「第七條:體力」三節,那條規則
容易被劇情檔悄悄違反,寫新劇情前先看——尤其「五條上限」這句話本身已經
過時,不要看到舊註解就以為現在還是五條。

**存檔(2026-09-09 加的)**:localStorage 自動存,靠 `game.html` 裡
`SAVE_OBJECTS`/`SAVE_LISTS`/`SAVE_VARS` 三張白名單登記表。**新增任務/NPC
狀態物件時要回去加一筆**——漏加不會報錯,只會安靜地不被存下來,玩家關掉
再開那條線就重置。理由與踩過的坑見 DESIGN_NOTES「本地存檔」。
